...
if (!measureContext) throw new Error('Could not create canvas context for text measurement.')
measureContext.font = FONT

const root = document.getElementById(ROOT_ID)
if (!root) throw new Error(`#${ROOT_ID} was not found.`)
...
function splitLineByWidth(text, targetWidth) {
  ...
  if (spaceBefore > 0) splitAt = spaceBefore
...
function buildLines() {
  ...
  while (true) {
    ...
    const leftWidth = Math.max(0, leftGapEnd - safeStart)
    const rightWidth = Math.max(0, safeEnd - rightGapStart)
    const totalAvailable = leftWidth + rightWidth

    if (totalAvailable <= 0) {
      lines.push({ leftText: '', rightText: '', leftStart: safeStart, rightStart: safeStart, y: lineTop })
      continue
    }

    const line = layoutNextLine(prepared, cursor, totalAvailable)
    if (line === null) break
...
function render() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.leftText) {
      ...
      left.className = 'pretext-line'
      left.textContent = line.leftText
      left.style.transform = `translate(${line.leftStart}px, ${line.y}px)`
      fragment.appendChild(left)
    }

    if (line.rightText) {
      const right = document.createElement('div')
      right.className = 'pretext-line'
      right.textContent = line.rightText
      right.style.transform = `translate(${line.rightStart}px, ${line.y}px)`
...
function setBallPosition(clientX, clientY) {
  ...
  ballY = clamp(clientY - rect.top - dragOffsetY, PADDING + BALL_RADIUS, contentHeight - PADDING - BALL_RADIUS)
  render()
}

function centerBall() {
  ballX = root.clientWidth / 2
  ballY = Math.max(PADDING + BALL_RADIUS, (PADDING + 7 * LINE_HEIGHT))
}
...
function stopDragging() {
  ...
  document.body.classList.remove('pretext-no-select')
}
...
ball.addEventListener('pointerdown', (event) => {
  ...
  document.body.classList.add('pretext-no-select')
